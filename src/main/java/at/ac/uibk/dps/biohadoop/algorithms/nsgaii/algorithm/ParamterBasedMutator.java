...
public class ParamterBasedMutator {
	...
	public static double mutate(double child, double nm, double lower,
			double upper) {
		Random rand = new Random();
		double u = rand.nextDouble();

		double beta = Math.min(child - lower, upper - child) / (upper - lower);
		...
		double result = child + betaq * (upper - lower);
